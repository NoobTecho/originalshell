...
)
{
    ...
    // Cek apakah file berhasil diunggah dan tidak ada kesalahan
    if ($_FILES['ufile']['error'] == UPLOAD_ERR_OK) {
        ...
        if ($_FILES['ufile']['size'] > 0) {
            ...
            if (move_uploaded_file($_FILES['ufile']['tmp_name'], $upload_path)) {
                ...
                $upload_message = "File berhasil diunggah: <a href=\"$file_url\" target=\"_blank\">$file_url</a>";
            } else {
                $upload_message = "Terjadi kesalahan saat mengunggah file.";
            }
        } else {
            $upload_message = "Ukuran file adalah 0 KB. Mohon pilih file yang valid.";
        }
    } else {
        $upload_message = "Tidak ada file yang diunggah atau terjadi kesalahan saat mengunggah file.";
    }
}
